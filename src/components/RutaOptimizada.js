...
// Punto base (puede cambiarse)
const puntoBase = { lat: -34.688263, lng: -58.546082 };

// Estilo tipo GPS Circuit / Waze
const mapOptions = {
  styles: [
    {
      featureType: "all",
...
const esAdmin = [
      ...
      async (result, status) => {
        if (status === "OK") {
          ...
          const pedidosOrdenados = orden.map((index, i) => {
            return { ...pedidos[index], ordenRuta: i + 1 };
          });
  ...
  if (pedidos.length === 0) return <p>📭 No hay pedidos asignados a este repartidor.</p>;
